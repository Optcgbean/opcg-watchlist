#!/bin/bash
# Update opcg-watchlist GitHub Pages with fresh data

cd /root/.openclaw/workspace/opcg-watchlist

# Initialize git if needed
if [ ! -d .git ]; then
    git init
    git remote add origin https://github.com/Optcgbean/opcg-watchlist.git
fi

# Configure git
 git config user.email "bot@opcg.watch" 2>/dev/null || true
 git config user.name "OPCG Bot" 2>/dev/null || true

# Copy fresh scrape data
cp /root/.openclaw/workspace/opcg_scrape_2026-05-22.json ./data.json 2>/dev/null || true

# Create/update script.js with fresh data
cat > script.js << 'JSEOF'
...
# Stage and commit
git add -A
git commit -m "Update: May 22, 2026 - 15-card watchlist prices" || true

# Push to GitHub
git push -u origin master --force || git push -u origin main --force || true

echo "Done updating GitHub Pages"
